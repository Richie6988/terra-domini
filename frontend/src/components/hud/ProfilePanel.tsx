    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ ...sBox, display: 'flex', gap: 16, alignItems: 'center' }}>
        <div style={{
          ...
          fontSize: 32, border: '3px solid rgba(255,255,255,0.08)',
          boxShadow: `0 4px 20px ${avatarBg}40, inset 0 -2px 6px rgba(0,0,0,0.1)`,
        }}><IconSVG id={avatar} size={32} /></div>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 16, fontWeight: 900, color: '#e2e8f0', letterSpacing: 1 }}>{player.display_name || player.username}</div>
          <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.4)', marginTop: 2 }}>@{player.username}</div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
          {AVATARS.map(a => (
            <button key={a} onClick={() => setAvatar(a)} style={{
              ...
              background: avatar === a ? `linear-gradient(135deg, ${avatarBg}20, ${avatarBg}10)` : 'rgba(255,255,255,0.03)',
              border: avatar === a ? `2px solid ${avatarBg}` : '2px solid rgba(255,255,255,0.05)',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
...
function AchievementsTab() {
  ...
  if (isLoading) return <div style={{ textAlign: 'center', padding: 30, color: 'rgba(255,255,255,0.25)', fontSize: 9 }}>Loading achievements...</div>
  if (!achievements.length) return <div style={{ textAlign: 'center', padding: 30, color: 'rgba(255,255,255,0.25)', fontSize: 9 }}>No achievements yet. Seed with: python manage.py seed_achievements</div>
...
function PreferencesTab() {
  ...
  const Tog=({l,v,f}:{l:string,v:boolean,f:(b:boolean)=>void})=>(
    <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center',padding:'8px 0' }}>
      ...
      <button onClick={()=>f(!v)} style={{ width:40,height:22,borderRadius:11,cursor:'pointer',border:'none',background:v?'#0099cc':'rgba(255,255,255,0.08)',padding:2,display:'flex',alignItems:'center',transition:'background 0.2s' }}>
