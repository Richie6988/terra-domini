  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        {state === 'full' && (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10 }}>⚔️</div>
        )}
      </div>
    </motion.div>
  )
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 3, cursor: 'default' }}>
        ...
        {slots_used > 0 && (
          <span style={{ fontSize: 9, color: '#F59E0B', fontFamily: 'monospace', marginLeft: 4, whiteSpace: 'nowrap' }}>
            {localSeconds > 0 ? `+1 in ${fmt(localSeconds)}` : '⚔️ ready'}
          </span>
        )}
      </div>
            ...
            <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: 8, marginTop: 4 }}>
              ...
              {stamina.attack_power_bonus > 0 && (
                <div style={{ fontSize: 10, color: '#FFB800' }}>
                  ⚔️ Attack power +{stamina.attack_power_bonus}%
                </div>
              )}
            </div>
