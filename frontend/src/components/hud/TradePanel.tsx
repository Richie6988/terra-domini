    ...
    <div style={{ padding: '16px 20px' }}>
      ...
      <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 12, padding: 16, marginBottom: 16 }}>
        ...
        <div style={{ marginBottom: 12 }}>
          <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.35)', letterSpacing: '0.08em', marginBottom: 6, textTransform: 'uppercase' }}>You give</div>
        ...
        {/* Buying */}
        <div>
          <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.35)', letterSpacing: '0.08em', marginBottom: 6, textTransform: 'uppercase' }}>You get</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 6 }}>
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <input type="number" value={amount} onChange={e => setAmount(Math.max(1, +e.target.value))} min={1}
            style={{ flex: 1, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 8, padding: '10px 12px', color: '#e2e8f0', fontSize: 16, fontFamily: 'monospace', outline: 'none', textAlign: 'center' }} />
        </div>
    ...
    <div style={{ padding: '16px 20px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: 8, alignItems: 'center', marginBottom: 16 }}>
        <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 10, padding: 12 }}>
          <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.35)', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.06em' }}>You offer</div>
          <select value={offer.resource} onChange={e => setOffer(p => ({...p, resource: e.target.value}))}
            style={{ width: '100%', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 6, padding: '6px', color: '#e2e8f0', fontSize: 12, marginBottom: 8 }}>
          ...
          <input type="number" value={offer.amount} onChange={e => setOffer(p => ({...p, amount: +e.target.value}))} min={1}
            style={{ width: '100%', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 6, padding: '6px 8px', color: '#e2e8f0', fontSize: 14, textAlign: 'center', boxSizing: 'border-box' }} />
        </div>
        <ArrowRightLeft size={20} color="#4B5563" />
        <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 10, padding: 12 }}>
          <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.35)', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.06em' }}>You want</div>
      ...
      {/* Player search */}
      <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.35)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 6 }}>Send to player</div>
      <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search username…"
        style={{ width: '100%', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, padding: '9px 12px', color: '#e2e8f0', fontSize: 13, outline: 'none', boxSizing: 'border-box', marginBottom: 8 }} />
      ...
      {Array.isArray(players) && players.slice(0, 5).map((p: any) => (
        <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', background: 'rgba(255,255,255,0.04)', borderRadius: 8, marginBottom: 6 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.35)' }}>Rank {p.commander_rank}</div>
          </div>
  ...
  return (
    <div style={{ padding: '16px 20px' }}>
      ...
      {RESOURCES.filter(r => r.id !== 'tdc').map(r => {
        ...
        const up = change > 0
        return (
          <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', background: 'rgba(255,255,255,0.04)', borderRadius: 10, marginBottom: 8 }}>
    ...
    <GlassPanel title="TRADE" onClose={onClose} accent="#22c55e">
      ...
      {activeKingdom && Object.keys(activeKingdom.dailyProduction).length > 0 && (
        <div style={{
          ...
          <div style={{
            fontSize:6, fontWeight:700, letterSpacing:2, color:'rgba(255,255,255,0.3)',
          ...
          <div style={{ display:'flex', flexWrap:'wrap', gap:3 }}>
            {Object.entries(activeKingdom.dailyProduction)
              ...
              <div key={res} style={{
                ...
                fontSize:6, fontWeight:700, color:'#e2e8f0',
