    ...
    <div style={{ display: 'flex', borderBottom: '1px solid rgba(255,255,255,0.07)', flexShrink: 0 }}>
      {tabs.map(t => (
        <button key={t.id} onClick={() => onChange(t.id)} style={{
          ...
          color: active === t.id ? '#00FF87' : '#6B7280',
    ...
    <div style={{ padding: '16px 20px' }}>
      ...
      <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 12, padding: 16, marginBottom: 16 }}>
        ...
        <div style={{ marginBottom: 12 }}>
          <div style={{ fontSize: 10, color: '#6B7280', letterSpacing: '0.08em', marginBottom: 6, textTransform: 'uppercase' }}>You give</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 6 }}>
            {RESOURCES.filter(r => r.id !== buying).map(r => (
              <button key={r.id} onClick={() => setSelling(r.id)} style={{
                padding: '8px 4px', borderRadius: 8, border: `1px solid ${selling === r.id ? r.color : 'rgba(255,255,255,0.08)'}`,
        ...
        {/* Buying */}
        <div>
          <div style={{ fontSize: 10, color: '#6B7280', letterSpacing: '0.08em', marginBottom: 6, textTransform: 'uppercase' }}>You get</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 6 }}>
            {RESOURCES.filter(r => r.id !== selling).map(r => (
              <button key={r.id} onClick={() => setBuying(r.id)} style={{
                padding: '8px 4px', borderRadius: 8, border: `1px solid ${buying === r.id ? r.color : 'rgba(255,255,255,0.08)'}`,
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <input type="number" value={amount} onChange={e => setAmount(Math.max(1, +e.target.value))} min={1}
            style={{ flex: 1, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 8, padding: '10px 12px', color: '#fff', fontSize: 16, fontFamily: 'monospace', outline: 'none', textAlign: 'center' }} />
        </div>
        <div style={{ textAlign: 'center', marginTop: 8, fontSize: 14, color: '#00FF87', fontWeight: 600 }}>
          {amount} {sellRes.emoji} → {youGet.toFixed(1)} {buyRes.emoji}
        </div>
      </div>

      <button onClick={() => tradeMut.mutate()} disabled={tradeMut.isPending}
        style={{ width: '100%', padding: '13px', background: 'rgba(0,255,135,0.15)', border: '1px solid rgba(0,255,135,0.4)', borderRadius: 12, color: '#00FF87', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>
    ...
    <div style={{ padding: '16px 20px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: 8, alignItems: 'center', marginBottom: 16 }}>
        <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 10, padding: 12 }}>
          <div style={{ fontSize: 10, color: '#6B7280', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.06em' }}>You offer</div>
          <select value={offer.resource} onChange={e => setOffer(p => ({...p, resource: e.target.value}))}
            style={{ width: '100%', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 6, padding: '6px', color: '#fff', fontSize: 12, marginBottom: 8 }}>
          ...
          <input type="number" value={offer.amount} onChange={e => setOffer(p => ({...p, amount: +e.target.value}))} min={1}
            style={{ width: '100%', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 6, padding: '6px 8px', color: '#fff', fontSize: 14, textAlign: 'center', boxSizing: 'border-box' }} />
        </div>
        <ArrowRightLeft size={20} color="#4B5563" />
        <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 10, padding: 12 }}>
          <div style={{ fontSize: 10, color: '#6B7280', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.06em' }}>You want</div>
      ...
      {/* Player search */}
      <div style={{ fontSize: 10, color: '#6B7280', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 6 }}>Send to player</div>
      <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search username…"
        style={{ width: '100%', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, padding: '9px 12px', color: '#fff', fontSize: 13, outline: 'none', boxSizing: 'border-box', marginBottom: 8 }} />

      {Array.isArray(players) && players.slice(0, 5).map((p: any) => (
        <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', background: 'rgba(255,255,255,0.03)', borderRadius: 8, marginBottom: 6 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 13, color: '#fff', fontWeight: 500 }}>{p.display_name || p.username}</div>
            <div style={{ fontSize: 10, color: '#6B7280' }}>Rank {p.commander_rank}</div>
          </div>
          <button onClick={() => proposeMut.mutate(p.id)}
            style={{ padding: '6px 12px', background: 'rgba(0,255,135,0.1)', border: '1px solid rgba(0,255,135,0.3)', borderRadius: 8, color: '#00FF87', cursor: 'pointer', fontSize: 12, fontWeight: 600 }}>
            Propose
          </button>
        </div>
      ))}
  ...
  return (
    <div style={{ padding: '16px 20px' }}>
      ...
      {RESOURCES.filter(r => r.id !== 'tdc').map(r => {
        ...
        const up = change > 0
        return (
          <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', background: 'rgba(255,255,255,0.03)', borderRadius: 10, marginBottom: 8 }}>
            <span style={{ fontSize: 24 }}>{r.emoji}</span>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 13, color: '#fff', fontWeight: 500 }}>{r.label}</div>
              <div style={{ fontSize: 10, color: '#4B5563' }}>per unit</div>
            </div>
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontSize: 16, fontWeight: 700, color: '#FFB800', fontFamily: 'monospace' }}>{rate.toFixed(1)} 🪙</div>
