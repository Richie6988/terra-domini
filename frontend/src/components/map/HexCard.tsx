...
export function HexCard({ territory:t, onClose, onRequestClaim, isNewClaim = false }:{
  ...
  // Empty hex with no POI → lightweight claim card (skip heavy Three.js)
  const isSimpleEmpty = isFree && !t.poi_category && !t.poi_name
    ...
    return (<>
      {createPortal(
        <div style={{
          ...
          background: 'rgba(26,42,58,0.5)', backdropFilter: 'blur(6px)',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
        }} onClick={onClose}>
          <div onClick={e => e.stopPropagation()} style={{
            width: '90%', maxWidth: 400, padding: '24px 28px', borderRadius: 16,
            background: 'linear-gradient(180deg, rgba(235,242,250,0.97), rgba(220,230,242,0.95))',
            border: '1px solid rgba(0,60,100,0.15)',
            boxShadow: '0 12px 40px rgba(0,0,0,0.2)',
          }}>
            <div style={{ textAlign: 'center', marginBottom: 16 }}>
              <div style={{ fontSize: 32, marginBottom: 8 }}>⬡</div>
              <div style={{ fontSize: 14, fontWeight: 900, color: '#1a2a3a', letterSpacing: 2, fontFamily: "'Orbitron', sans-serif" }}>
                UNCLAIMED TERRITORY
              </div>
              <div style={{ fontSize: 9, color: 'rgba(26,42,58,0.4)', marginTop: 4, fontFamily: "'Share Tech Mono', monospace" }}>
                {t.h3_index?.slice(0, 15)} · {(biome || 'rural').toUpperCase()}
              </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {claimOpts?.locked && (
                <div style={{ padding: 12, borderRadius: 10, background: 'rgba(100,100,100,0.06)', border: '1px solid rgba(100,100,100,0.15)', textAlign: 'center', color: '#9CA3AF', fontSize: 9, fontWeight: 700 }}>
                  🔒 TERRITORY LOCKED — must be adjacent to your kingdom
                </div>
              )}
              {!claimOpts?.locked && claimOpts?.options?.map((opt: any) => (
                <button key={opt.method} onClick={() => handleClaim(opt.method)} disabled={claiming || !opt.available}
                  style={{
                    padding: '12px 16px', borderRadius: 10, cursor: opt.available ? 'pointer' : 'not-allowed',
                    background: opt.method === 'free' ? 'linear-gradient(90deg, #22c55e, #16a34a)' : opt.method === 'buy' ? 'linear-gradient(90deg, #cc8800, #b37700)' : 'linear-gradient(90deg, #0099cc, #0077aa)',
                    border: 'none', color: '#fff', fontSize: 10, fontWeight: 700, letterSpacing: 2,
                    fontFamily: "'Orbitron', sans-serif", opacity: opt.available ? 1 : 0.4,
                    display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                  }}>
                  <span>{opt.method === 'free' ? '🆓 CLAIM FREE' : opt.method === 'buy' ? '💰 BUY NOW' : '🔍 EXPLORE'}</span>
                  <span style={{ fontFamily: "'Share Tech Mono', monospace" }}>{opt.cost > 0 ? `${opt.cost} ◆` : 'FREE'}</span>
                </button>
              ))}
              {!claimOpts && !claiming && (
                <div style={{ textAlign: 'center', padding: 12, color: 'rgba(26,42,58,0.3)', fontSize: 9 }}>Loading claim options...</div>
              )}
            </div>

            <button onClick={onClose} style={{
              width: '100%', marginTop: 12, padding: '8px', borderRadius: 8, cursor: 'pointer',
              background: 'transparent', border: '1px solid rgba(0,60,100,0.1)',
              color: 'rgba(26,42,58,0.4)', fontSize: 8, fontWeight: 600, letterSpacing: 2,
            }}>CLOSE</button>
          </div>
        </div>,
        document.body
      ...
      {celebrating && createPortal(
        <ClaimCelebration visible={celebrating} territoryName={cardName.toUpperCase()} rarity={rarity}
