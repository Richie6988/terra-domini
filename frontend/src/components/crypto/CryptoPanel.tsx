  ...
  return (
    <div style={{ display: 'flex', gap: 8, overflowX: 'auto', padding: '10px 0', scrollbarWidth: 'none' }}>
      {Object.entries(prices).map(([sym, p]) => {
        ...
        return (
          <div key={sym} style={{ flexShrink: 0, background: 'rgba(255,255,255,0.04)', borderRadius: 10, padding: '8px 14px', border: `1px solid ${sym === 'HEX' ? 'rgba(0,255,135,0.25)' : 'rgba(255,255,255,0.07)'}`, minWidth: 90 }}>
            <div style={{ fontSize: 10, color: '#6B7280', letterSpacing: '0.1em' }}>{sym}</div>
            <div style={{ fontSize: 14, fontWeight: 700, color: sym === 'HEX' ? '#00FF87' : '#fff', fontFamily: 'monospace', marginTop: 2 }}>
  ...
  return (
    <div style={{ background: 'linear-gradient(135deg, rgba(0,255,135,0.07) 0%, rgba(139,92,246,0.07) 100%)', border: '1px solid rgba(0,255,135,0.18)', borderRadius: 16, padding: 20, marginBottom: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
        <div>
          <div style={{ fontSize: 10, color: '#6B7280', textTransform: 'uppercase', letterSpacing: '0.1em' }}>HEX Balance</div>
          <div style={{ fontSize: 28, fontWeight: 800, color: '#00FF87', fontFamily: 'monospace', marginTop: 2 }}>
            {toF(wallet?.tdi_balance, 4)}
            <span style={{ fontSize: 13, color: '#6B7280', marginLeft: 6 }}>HEX</span>
          </div>
          <div style={{ fontSize: 12, color: '#9CA3AF', marginTop: 3 }}>≈ ${toF(wallet?.tdi_usd_value, 2)} USD</div>
        </div>
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontSize: 10, color: '#6B7280' }}>In-Game HEX Coin</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
        <button onClick={onConvert} style={{ padding: 10, background: 'rgba(0,255,135,0.12)', border: '1px solid rgba(0,255,135,0.3)', borderRadius: 10, color: '#00FF87', cursor: 'pointer', fontSize: 12, fontWeight: 700 }}>⇄ HEX → HEX Coin</button>
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 2000, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
      <motion.div initial={{ y: 300 }} animate={{ y: 0 }} style={{ background: '#0F0F1A', borderRadius: '20px 20px 0 0', padding: 28, width: '100%', maxWidth: 420, border: '1px solid rgba(255,255,255,0.08)' }}>
        <div style={{ fontSize: 17, fontWeight: 700, color: '#fff', marginBottom: 6 }}>Convert HEX → HEX Coin</div>
        <div style={{ fontSize: 11, color: '#6B7280', marginBottom: 14 }}>Rate: 1 HEX ≈ {rate.toFixed(1)} HEX Coin · Available: {toF(wallet?.tdi_balance, 4)} HEX</div>
        ...
        <div style={{ textAlign: 'center', color: '#00FF87', fontSize: 16, fontWeight: 700, marginBottom: 16 }}>→ 🪙 {preview.toFixed(1)} HEX Coin</div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px' }}>
          ...
          {tab === 'markets' && Object.entries(prices).map(([sym, p]: any) => (
            <div key={sym} style={{ display: 'flex', alignItems: 'center', padding: '14px 0', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
              <div style={{ width: 40, height: 40, borderRadius: '50%', background: sym === 'HEX' ? 'rgba(0,255,135,0.12)' : 'rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, marginRight: 14, flexShrink: 0 }}>
              ...
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontSize: 15, fontFamily: 'monospace', fontWeight: 700, color: sym === 'HEX' ? '#00FF87' : '#fff' }}>
      ...
      {showWithdraw && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 2000, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
          <motion.div initial={{ y: 300 }} animate={{ y: 0 }} style={{ background: '#0F0F1A', borderRadius: '20px 20px 0 0', padding: 28, width: '100%', maxWidth: 420 }}>
            <div style={{ fontSize: 17, fontWeight: 700, color: '#fff', marginBottom: 4 }}>Withdraw HEX</div>
